...
import TaskRecord from '../dataSchemas/TestResults.js';
import EvalMetrics from '../dataSchemas/Assignment.js';
import Learner from '../dataSchemas/Student.js';
...
router.post('/upload-assignment',asyncHandler(async (req, res)=>{
    try {
        ...
        const newTaskRecord = new TaskRecord({ title, questions });
    
        await newTaskRecord.save();
    
        res.status(201).json({ message: 'TaskRecord successfully uploaded', assignment: newTaskRecord });
      } catch (error) {
...
router.get('/all-assignments', asyncHandler( async (req, res) => {
  ...
  try {
    const assignments = await TaskRecord.find().sort({ dueDate: 1 }); // Sorting by due date
...
router.post('/submit-test', asyncHandler(async (req, res)=> {
  ...
  try{
    ...
    const newTestResult = new EvalMetrics({
      assignmentId,
      rollNo,
      totalScore,
...
//     const student = await Learner.findOne({ rollNo });
...
router.get('/attempted-tests/:rollNo', async (req, res) => {
  try {
    ...
    const attemptedTests = await EvalMetrics.find({ rollNo }).distinct('assignmentId'); // Get only assignment IDs
...
router.get('/results/:assignmentId/:rollNo', async (req, res) => {
  try {
    ...
    const result = await EvalMetrics.findOne({ assignmentId, rollNo });
...
router.get('/results/:rollNo', async (req, res) => {
  try {
      ...
      const results = await EvalMetrics.find({ rollNo }).populate('assignmentId', 'title');
